...
export function ConceptLogoIcon({ size = 'md', className }: { size?: 'sm' | 'md' | 'lg'; className?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* Optimized Precision Vector SVG */}
      <svg
        viewBox="0 0 32 32"
        fill="none"
      ...
      >
        {/* Geometric Bracket Invariant (Outer C Frame) */}
        <path
        ...
        />
        {/* Inner Cognitive Core Node (Ochre Mental Model) */}
...
export function Logo({
  ...
  if (asLink) {
    return (
      <Link href="/" className="inline-block" aria-label="Concept Home">
        {content}
      </Link>
    );
  }
