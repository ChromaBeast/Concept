...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-50 grid w-full max-w-xl translate-x-[-50%] translate-y-[-50%] gap-4 border border-paper-border bg-paper-card p-6 shadow-2xl duration-200 data-[state=open]:animate-fadeIn rounded-2xl font-sans max-h-[90vh] overflow-y-auto',
        className
      )}
      {...props}
    >
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={cn('text-lg font-bold leading-none tracking-tight text-paper-text font-sans', className)}
    {...props}
  />
));
